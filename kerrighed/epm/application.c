...
struct app_struct* new_local_app(long app_id)
{
	...
	memset(app, 0, sizeof(struct app_struct));
...
int app_set_userdata(__u64 user_data, credentials_t *creds)
{
	int r = 0;
	struct app_kddm_object *obj;

	if (!can_be_checkpointed(current, creds)) {
		r = -EPERM;
		goto exit;
	}
		     ...
		     credentials_t *creds)
{
	...
	if (type_id == FROM_PID)
		app_id = get_appid_from_pid(_appid, creds);
