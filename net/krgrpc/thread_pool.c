...
void do_krgrpc_handler(struct rpc_desc* desc,
 ...
 continue_in_synchro:
	...
	if (test_bit(desc->rpcid, static_communicator.rpc_mask)) {
...
int thread_pool_run(void* _data){
	...
	continue_in_waiting_desc:
		...
		list_for_each_entry_safe(wd, wd_safe,
			...
			if (test_bit(wd->desc->rpcid, static_communicator.rpc_mask))
				continue;
...
int rpc_handle_new(struct rpc_desc* desc){
	...
	if (unlikely(test_bit(desc->rpcid, static_communicator.rpc_mask))) {
